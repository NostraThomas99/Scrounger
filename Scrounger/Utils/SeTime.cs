...
public class SeTime
{
    ...
    private void Update(IFramework _)
    {
        ...
        if (hour != EorzeaTotalHour)
        {
            ...
            {
                Scrounger.Log.Verbose($"Eorzea Hour and Weather Change triggered. {(long)ServerTime} {(long)EorzeaTime}");
                WeatherChanged?.Invoke();
            }
            else
            {
                Scrounger.Log.Verbose($"Eorzea Hour Change triggered. {(long)ServerTime} {(long)EorzeaTime}");
            }
        }
