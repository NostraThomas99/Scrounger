...
namespace Scrounger.AutoGather.Movement
{
    ...
    public sealed class AdvancedUnstuck : IDisposable
    {
        ...
        public AdvancedUnstuckCheckResult Check(Vector3 destination, bool isPathGenerating, bool isPathing)
        {
            ...
            if (now.Subtract(_unstuckStart).TotalSeconds < Scrounger.Config.NavResetCooldown
                || destination == default
            ...
            else if (isPathing)
            {
                ...
                else if (now.Subtract(_lastMovement).TotalSeconds > Scrounger.Config.NavResetThreshold)
                {
                    Scrounger.Log.Warning($"Advanced Unstuck: the character is stuck. Moved {_lastPosition.DistanceToPlayer()} yalms in {now.Subtract(_lastMovement).TotalSeconds} seconds.");
                    Start();
                }
            }
            ...
            {
                Scrounger.Log.Warning($"Advanced Unstuck: vnavmesh failure detected.");
                Start();
            }
        ...
        public void Force()
        {
            if (!IsRunning)
            {
                Scrounger.Log.Warning("Advanced Unstuck: force start.");
                Start();
            }
        }
