...
using Actions = Scrounger.AutoGather.AutoGather.Actions;
    ...
    internal static class RotationSolver
    {
        ...
        private static void SolveInternal(State state)
        {
            if (state.Integrity == 0)
            {
                ...
                else
                {
                    ...
                    {
                        //Scrounger.Log.Debug($"Rotation solver: new best. Iteration {state.Global.Iterations}. Yield {state.TotalYield/1000m} vs. {state.Global.BestYield/1000m}. GP {state.GP} vs. {state.Global.BestGP}. Sequence: {string.Join(", ", state.Global.Actions.Reverse().Select(a => a?.Name ?? "Gather"))}");
        ...
        public static async Task<IEnumerable<Actions.BaseAction?>> SolveAsync(ItemSlot slot, ConfigPreset config)
        {
            ...
            var global = new GlobalState()
            {
                ...
                OptimizeForCost = slot.Item.NodeType == GatherBuddy.Enums.NodeType.Regular,
            ...
            if (slot.Item.NodeType is GatherBuddy.Enums.NodeType.Unspoiled or GatherBuddy.Enums.NodeType.Legendary)
            {
                await Task.Run(() => SolveInternal(state));
            }
            else if (slot.Item.NodeType is GatherBuddy.Enums.NodeType.Regular)
            {
            ...
            timer.Stop();
            Scrounger.Log.Debug($"Rotation solver: simulated {global.Iterations} sequences in {timer.ElapsedTicks * 1000m / Stopwatch.Frequency:F3} ms. Yield: {global.BestYield / 1000m}. Sequence: {string.Join(", ", global.BestActions.Select(a => GetActionName(global, a)))}.");
        ...
        private static async Task SolveForRegularNodes(State state)
        {
            ...
            if (bountifulYield < 3000 || bounty != null)
            {
                ...
                if (state.Global.SpendGPOnBestNodesOnly)
                {
                    ...
                    Scrounger.Log.Debug($"Rotation solver: yield / 100gp current: {state.Global.BestYield / 1000m}; integrity+2: {stateIntegrity.Global.BestYield / 1000m}; yield+3: {stateYield.Global.BestYield / 1000m}; boon+100: {stateBoon.Global.BestYield / 1000m}; filler {fillerYield / 1000m}.");
                ...
                {
                    await Task.Run(() => SolveInternal(state));
                    Scrounger.Log.Debug($"Rotation solver: yield / 100gp current: {state.Global.BestYield / 1000m}; filler {fillerYield / 1000m}.");
                }
            }
            ...
            if (state.Global.BestActions.Count == 0 || state.Global.BestYield <= fillerYield || state.Global.BestYield < bestSimulatedYield * (100 - MaxYieldDiffPercent) / 100)
            {
                ...
                var canUseGiving = (isCrystal || Scrounger.Config.UseGivingLandOnCooldown)
        ...
        private static unsafe float TheGivingLandCooldown
        {
            get
            {
                ...
                {
                    Scrounger.Log.Error("BUG: RotationSolver.TheGivingLandCooldown is accessed from a worker thread.");
                    return 0f;
                }
        ...
        private static uint CalculateBoonChance(uint glvl)
        {
            if (!Svc.Framework.IsInFrameworkUpdateThread)
            {
                Scrounger.Log.Error("BUG: RotationSolver.CalculateBoonChance is accessed from a worker thread.");
                return 0;
            }
            var basePerception = Scrounger.WorldData.IlvConvertTable[(int)glvl].BasePerception;
