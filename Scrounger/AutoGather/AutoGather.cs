...
using Scrounger.AutoGather.Movement;
...
namespace Scrounger.AutoGather
{
    ...
    {
        public AutoGather(Scrounger plugin)
        {
        ...
        public bool GoHome()
        {
            ...
            {
                Scrounger.Log.Warning("Lifestream not found or not ready");
                return false;
            }
        }
        ...
        public void DoAutoGather(IFramework framework)
        {
            if (!IsGathering)
            ...
            if (!CanAct)
            {
                AutoStatus = Svc.Condition[ConditionFlag.Gathering] ? "Gathering..." : "Player is busy...";
                return;
            }
                ...
                if (gatherTarget != default)
                {
                    ...
                    if (target != null && target.ObjectKind is ObjectKind.GatheringPoint)
                    {
                        ...
                        if (gatherTarget.Item.NodeType is NodeType.Regular or NodeType.Ephemeral
                            ...
                            && Scrounger.WorldData.NodeLocations[gatherTarget.Node.Id].ContainsKey(target.DataId) == true)
                        {
                            FarNodesSeenSoFar.Clear();
                            VisitedNodes.AddLast(target.DataId);
                            while (VisitedNodes.Count > (Scrounger.WorldData.NodeLocations[gatherTarget.Node.Id].Count <= 4 ? 2 : 4))
                                VisitedNodes.RemoveFirst();
                        }
                    }
                }

                if (!Scrounger.Config.DoGathering)
                    return;
            ...
            if (next == default)
            {
                ...
                if (Scrounger.Config.GoHomeWhenIdle)
                    if (GoHome())
                        return;
            ...
            if (territoryId == 478 && next.Node.Territory.Id == 399 && Lifestream.Enabled)
            {
                ...
                if (aetheryte != null)
                {
                    ...
                    else if (!Lifestream.IsBusy())
                    {
                        ...
                        var name = Svc.Data.GetExcelSheet<Lumina.Excel.Sheets.Aetheryte>().GetRow(exit).AethernetName.Value.Name
                            .ToString();
            ...
            if (forcedAetheryte.ZoneId != 0
             && (Scrounger.WorldData.Aetherytes[forcedAetheryte.AetheryteId].Territory.Id == territoryId
            ...
            var allPositions = Scrounger.WorldData.NodeLocations
                .Where( nl => nl.Key == next.Node.Id)
                .ExceptBy(VisitedNodes, n => n.Key)
                .SelectMany(w => w.Value.Values)
                .SelectMany(l => l)
            ...
            {
                Scrounger.Log.Debug("Current destination doesn't match the target location, resetting navigation");
                StopNavigation();
                FarNodesSeenSoFar.Clear();
                VisitedNodes.Clear();
            }
            ...
            if (CurrentDestination != default)
            {
                ...
                if (currentNode != null && !currentNode.IsTargetable)
                    Scrounger.Log.Verbose($"Far node is not targetable, distance {currentNode.Position.DistanceToPlayer()}.");
                ...
                {
                    Scrounger.Log.Verbose("Far node is not targetable, choosing another");
                }
                else
                {
                    return;
            ...
            if (ShouldUseFlag && next.Item.NodeType is NodeType.Legendary or NodeType.Unspoiled)
            {
                ...
                if (pos == null || next.Time.Start > Scrounger.Time.ServerTime.AddSeconds(-8))
                {
            ...
            else
            {
                ...
                {
                    FarNodesSeenSoFar.Clear();
                    Scrounger.Log.Verbose($"Selected node was null and far node filters have been cleared");
                    return;
                }
            }
        ...
        private void AbortAutoGather(string? status = null)
        {
            if (!string.IsNullOrEmpty(status))
                AutoStatus = status;
            if (Scrounger.Config.HonkMode)
                Task.Run(() => _soundHelper.PlayHonkSound(3));
            CloseGatheringAddons();
            if (Scrounger.Config.GoHomeWhenDone)
        ...
        private bool CheckCollectablesUnlocked(GatheringType gatheringType)
        {
            ...
            {
                ChatPrinter.PrintError("You've put a collectable on the gathering list, but your level is not high enough to gather it.");
                return false;
            }
            ...
            {
                ChatPrinter.PrintError("You've put a collectable on the gathering list, but you haven't unlocked the collectables.");
                ...
                text.AddText("Collectables are unlocked by ")
                    ...
                    .AddUiForegroundOff()
                    .AddText(".");
                ChatPrinter.Print(text.BuiltString);
                return false;
            }
        ...
        private bool ChangeGearSet(GatheringType job)
        {
            var set = job switch
            {
                GatheringType.Miner => Scrounger.Config.MinerSetName,
                GatheringType.Botanist => Scrounger.Config.BotanistSetName,
                _ => null,
            };
            if (string.IsNullOrEmpty(set))
            {
                ChatPrinter.PrintError($"No gear set for {job} configured.");
                return false;
            }
