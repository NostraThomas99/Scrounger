...
namespace Scrounger.AutoGather
{
    ...
    {
        public static readonly Item[] PossibleCordials = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemCordial).ToArray() ?? [];

        public static readonly Item[] PossibleFoods = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemDoLFood).ToArray() ?? [];

        public static readonly Item[] PossiblePotions = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemDoLPotion).ToArray() ?? [];

        public static readonly Item[] PossibleManuals = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemDoLManual).ToArray() ?? [];
        ...
        public static readonly Item[] PossibleSquadronManuals = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemDoLSquadronManual).ToArray() ?? [];

        private static readonly Dictionary<uint, uint> SquadronPassItemIdBuffId = new ()
        {
            { 14954u, 1061u }
        };
        public static readonly Item[] PossibleSquadronPasses = Svc.Data.GetExcelSheet<Item>()?.Where(IsItemDoLSquadronPass).ToArray() ?? [];
        ...
        private static uint[] GetItemFoodProps(Item item)
        {
            ...
            if (item.ItemUICategory.RowId is not 44 and not 46
                ...
                return [];
            return Svc.Data.GetExcelSheet<ItemFood>().GetRow(item.ItemAction.Value.Data[1]).Params.Where(v => v.BaseParam.IsValid).Select(v => v.BaseParam.RowId).ToArray() ?? [];
        }
        ...
        {
            var buff = Svc.ClientState?.LocalPlayer?.StatusList.FirstOrDefault(s => s.StatusId == 48);
            if (buff == null)
            {
                return false;
            }
        ...
        {
            var buff = Svc.ClientState?.LocalPlayer?.StatusList.FirstOrDefault(s => s.StatusId == 49);
            if (buff == null)
            {
                return false;
            }
        ...
        public unsafe bool IsManualBuffUp => Svc.ClientState?.LocalPlayer?.StatusList.Any(s => s.StatusId == 46) ?? false;
        ...
        public unsafe bool GetIsSquadronManualBuffUp(uint itemId)
        {
            ...
            {
                return Svc.ClientState?.LocalPlayer?.StatusList.Any(s => s.StatusId == requiredBuffId) ?? false;
            }
            else
            {
                return false;
