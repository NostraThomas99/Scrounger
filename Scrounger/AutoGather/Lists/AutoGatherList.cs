...
public class AutoGatherList
{
    ...
    {
        return Utils.Functions.Move(items, from, to);
    }
    ...
    public struct Config(AutoGatherList list)
    {
        ...
        internal readonly string ToBase64()
        {
            ...
            return Utils.Functions.CompressedBase64(bytes);
        }
        ...
        internal static bool FromBase64(string data, out Config cfg)
        {
            ...
            try
            {
                var bytes = Utils.Functions.DecompressedBase64(data);
    ...
    public static bool FromConfig(Config cfg, out AutoGatherList list)
    {
        ...
        {
            uint quantity;
            if (Scrounger.WorldData.Gatherables.TryGetValue(itemId, out var item) && list.Add(item, quantity = cfg.Quantities.GetValueOrDefault(item.ItemId)))
            {
