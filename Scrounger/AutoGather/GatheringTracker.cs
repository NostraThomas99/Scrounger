...
namespace Scrounger.AutoGather
{
    public sealed class GatheringTracker : IDisposable, IReadOnlyList<ItemSlot>
    {
        ...
        public sealed class ItemSlot
        {
            ...
            public Gatherable Item
            {
                get
                {
                    ...
                    if (Scrounger.WorldData.Gatherables.TryGetValue(Id, out var gatherable))
                    {
                        cachedItem = gatherable;
                        return cachedItem;
                    }

                    return Scrounger.WorldData.Gatherables.Values.First();
                }
            }
        ...
        private void ProcessArgs(Span<AtkValue> values)
        {
            ...
            {
                Scrounger.Log.Debug($"{GetType()}: unexpected value of argument {n}: {values[n].ToString()}.");
            }
        }
