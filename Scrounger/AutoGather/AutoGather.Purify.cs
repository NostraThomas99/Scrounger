...
namespace Scrounger.AutoGather
{
    public partial class AutoGather
    {
        ...
        {
            if (!Scrounger.Config.DoReduce || Svc.Condition[ConditionFlag.Mounted])
                return false;

            if (!QuestManager.IsQuestComplete(67633)) // No Longer a Collectable
            {
                Scrounger.Config.DoReduce = false;
                ChatPrinter.PrintError(
                    "[Scrounger] Aetherial reduction is enabled, but the relevant quest has not been completed yet. The feature has been disabled.");
                return false;
            }
            ...
            unsafe
            {
                ...
                foreach (var invType in InventoryTypes)
                {
                    ...
                    for (int i = 0; i < container->Size; i++)
                    {
                        ...
                        if (slot != null
                         && slot->ItemId != 0
                         && Scrounger.WorldData.Gatherables.TryGetValue(slot->ItemId, out var gatherable)
        ...
        private unsafe void ReduceItems(bool reduceAll)
        {
            ...
            var delay = (int)Scrounger.Config.ExecutionDelay;
